 ...
 * The signed URL (7-day validity) allows the owner to download the ZIP
 ...
 *
 * ShouldBeUnique: prevents duplicate "export ready" emails for the same org
 * from flooding the queue (e.g. if the command is accidentally run twice).
 * Lock TTL: 1 hour per organization.
 */
class OrganizationDataExportReadyNotification extends Notification implements ShouldBeUnique, ShouldQueue
{
    use Queueable;
    ...
    {
        return 'data-export:'.$this->organizationId;
    }
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ...
            ->line('Link do pobrania jest ważny przez **7 dni**. Po tym czasie należy wygenerować nowy eksport.')
