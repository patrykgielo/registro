...
| 9 | **Dane konta tenanta** (`organizations`, `users` właściciela) | Czas trwania umowy + okres rozliczeniowy/roszczeń | Umowa (Art. 6(1)(b) RODO); Art. 118 KC | ✅ Soft-delete org (zwrot danych: osobny eksport, wiersz 14). 🔴 Usunięcie/anonimizacja **kont userów właściciela** NIE zaimplementowane — dług | Soft-delete org w `PurgeClosedOrganizationsCommand` (krok 3); usuwanie kont userów = brak |
