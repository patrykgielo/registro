...
class RentalExtensionService
{
     ...
     *
     * $forUpdate = true makes the underlying rentals/order_items count queries
     * locking reads (see RentalAvailabilityService::getAvailableQuantity()'s
     * docblock for why this — not just locking the Service row — is what
    ...
    public function requestExtension(
        ...
        return DB::transaction(function () use ($item, $user, $requestedEndDate, $notes) {
            ...
            // Lock the Service row too — concurrent requestExtension()/approve() calls for
    ...
    public function approve(OrderItemExtensionRequest $extensionRequest, User $admin): void
    {
        DB::transaction(function () use ($extensionRequest, $admin) {
            ...
            // queries below still close the race on their own (see getAvailableQuantity()'s
            // docblock), but locking the Service row here also serialises this approve()
            // against a concurrent requestExtension()/approve() for the SAME service.
            ...
            // Update order totals — total_amount/subtotal are normally immutable on Order;
            // this is the sanctioned escape hatch (see Order::applyFinancialAdjustment()).
            // Re-fetch with a row lock (matches Przelewy24Service::handleWebhook()'s pattern)
            // instead of reusing the lazy-loaded $extensionRequest->order: without this, two
            // approve() calls for different items on the same order can both read the same
            // starting subtotal/total_amount and the second save silently overwrites the
            // first's increment (lost update).
            $order = Order::where('id', $extensionRequest->order_id)->lockForUpdate()->first();
            $order->applyFinancialAdjustment(
                ['subtotal' => (float) $extensionRequest->additional_amount, 'total_amount' => (float) $extensionRequest->additional_amount],
