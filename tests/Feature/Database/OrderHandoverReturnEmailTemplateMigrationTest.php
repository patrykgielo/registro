 ...
 * provisioning" gate).
 *
 * Runs entirely against SQLite via the normal test harness (.env.testing) — never
 * touches dev MySQL.
 */
class OrderHandoverReturnEmailTemplateMigrationTest extends TestCase
{
    use RefreshDatabase;

    private const MIGRATION_PATH = 'database/migrations/2026_08_12_120000_seed_order_handover_return_email_templates.php';

    private const NEW_KEYS = ['order-handed-over', 'order-returned'];
    ...
    public function test_up_inserts_exactly_four_global_rows(): void
    {
        ...
        $this->assertEqualsCanonicalizing(
            ['order-handed-over:pl', 'order-handed-over:en', 'order-returned:pl', 'order-returned:en'],
    ...
    /**
     * A tenant that already customized its OWN override of one of these brand-new
     * keys (edge case, but the composite unique makes it possible) must survive
    ...
    /**
     * The exact class of bug flagged in review: EmailTemplateSeeder gets a new
     ...
     *
     * Scope: only asserts on the two keys this branch introduces. A parallel
     * repo-wide audit while writing this test found `order-paid`,
     * `order-confirmed`, `order-cancelled`, `admin-new-order`, `rental-cancelled`
     * and `service-area-available` are ALSO missing from any production data
     * migration — a real, pre-existing instance of this same bug class. Reported
     * separately; deliberately not fixed here (out of this branch's scope, and
     * each deserves its own reviewed change, not a drive-by bundled in here).
     */
    public function test_both_new_template_keys_resolve_from_production_migrations_alone(): void
    {
        ...
        foreach (['order-handed-over', 'order-returned'] as $key) {
