...
<x-layout.section spacing="sm">
    <div class="flex gap-8 lg:gap-12 items-start">
        ...
        <aside
            ...
            aria-label="Kategorie portfolio"
        >
                ...
                <nav>
                    <ul role="list">
                        @foreach($allCategories as $cat)
                            ...
                            <li>
                                <a
                                    href="{{ route('portfolio.category', $cat) }}"
        ...
        {{-- ─────────────────────────────────────────────────────────────────
             ...
             ───────────────────────────────────────────────────────────────── --}}
        <div class="lg:hidden -mx-4 sm:-mx-6 mb-6 w-screen px-4 sm:px-6" aria-label="Kategorie portfolio">
            ...
            @if($items->isNotEmpty())
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($items as $item)
                        <x-cms.card :item="$item" :url="route('portfolio.show', $item->slug)" :dark="false" />
                    @endforeach
                </div>
            ...
            @else
                ...
                <div class="text-center py-16 rounded-xl border border-border bg-surface-raised">
                    ...
                    <h3 class="text-lg font-semibold text-text-primary mb-2">
                        Brak dostępnych realizacji
                    </h3>
                    <p class="text-text-secondary text-sm mb-6">
                        Brak dostępnych realizacji w kategorii „{{ $category->name }}".
                    </p>
                    <x-ui.button
                        variant="secondary"
                        href="{{ route('home') }}"
