...
@php
    ...
    $hasDropdownContent = $canSwitchTenants || $itemsBeforeTenantSwitcher->isNotEmpty() || $itemsAfterTenantSwitcher->isNotEmpty();
@endphp

{{ \Filament\Support\Facades\FilamentView::renderHook(\Filament\View\PanelsRenderHook::TENANT_MENU_BEFORE) }}

@if ($hasDropdownContent)
    {{-- User has multiple tenants or menu items — show dropdown with switcher --}}
    <x-filament::dropdown
...
@else
    {{-- Single tenant — show name without dropdown/chevron --}}
    <div
        :attributes="
            \Filament\Support\prepare_inherited_attributes($attributes)
                ->class(['fi-tenant-menu'])
