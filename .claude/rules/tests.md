...
Runs in ~17 s. Most of it is offline; four cases start real containers because
...
./appendonlydir: Permission denied` verbatim), and
`37_horizon_storage_public_mount.sh` (`alpine:3`, a real named volume seeded
the way `app` actually writes it — a text match on "storage-app-public"
would pass the instant the string appeared in a comment, without proving a
container mounted at that exact path can see a file `app` wrote there;
negative control is a container with no mount at all, reproducing the exact
silent-failure symptom — ClickUp 123k99ct3za, `ci-cd-troubleshooting.md`).
