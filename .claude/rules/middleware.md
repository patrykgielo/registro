...
- `ShareSelectedLocation` (Faza 5.1, `bootstrap/app.php` — globalna grupa `web`, zaraz po `CheckMaintenanceMode`) - czyści `session('selected_location_id')`, gdy nie rozwiązuje się do aktywnej lokalizacji BIEŻĄCEGO tenanta (obcy tenant, usunięta, nieaktywna). Nigdy nie rzuca, nigdy nie resolvuje tenanta z sesji — tylko z `TenantFeature::currentTenant()` (request attribute). Logika w `App\Support\LocationContext`, samodzielnie bezpiecznej nawet bez tego middleware (rewalidacja przy każdym odczycie). Szczegóły: `app/docs/features/lokalizacje/README.md`.
- `RequireTenant` (`require.tenant`) - `abort(404)` gdy `$request->attributes->get('tenant') === null`. **KRYTYCZNE (VULN-003)**: `ResolveTenant` na root domain celowo NIE ustawia `tenant` — `BelongsToOrganization` scope wtedy no-opuje. KAŻDA trasa query-ująca `BelongsToOrganization` (w tym `routes/api.php` — `api` group NIE ma `ResolveTenant` domyślnie!) MUSI mieć `RequireTenant::class` zaraz PO `ResolveTenant::class`. Szczegóły + 2 gap-fixy: `app/docs/security/vulnerabilities/VULN-003-root-domain-tenant-bypass.md`.
  - **NIGDY** `TenantFeature::currentTenant()` w `RequireTenant` — zawsze `$request->attributes->get('tenant')`. Do Layer 8 chodziło o session fallback (zapisywany dla KAŻDEGO gościa PRZED `canAccessTenant()`); ta gałąź zniknęła z każdego realnego requestu (zostaje wyłącznie wąska furtka test-only, patrz Layer 8 niżej), ale reguła zostaje — request attribute jest jedynym sygnałem scoped do TEGO requestu.
  ...
  - **Layer 8 (2026-08-31): `TenantFeature::currentTenant()`'s 3. (session) gałąź usunięta z KAŻDEGO realnego żądania.** `session('tenant_id')` był zapisywany na KAŻDEJ udanej wizycie subdomeny, w tym anonimowej, i przeżywał powrót na domenę główną w tej samej przeglądarce — ktokolwiek wołał `currentTenant()` z trasy osiągalnej z root domain (np. stopka `home-fallback` przez `NavigationService`/`Page`) dziedziczył cudzego tenanta. `ResolveTenant` nadal ZAPISUJE ten klucz (pinowane testy `LivewireAdminTenantIsolationTest` asertują na tym jako sygnale). Odczyt zostaje wyłącznie jako wąska furtka test-only (`app()->runningUnitTests()` ORAZ brak `tenant_resolution_attempted` — czyli `Livewire::test()`, które nigdy nie przechodzi przez kernel HTTP; ~20 istniejących testów zasobów Filament używa `session(['tenant_id' => ...])` jako jedynego sposobu na "acting as tenant X" w takim teście), strukturalnie nieosiągalna poza `APP_ENV=testing` i zawsze pomijana, gdy `ResolveTenant` faktycznie zadziałał dla requestu. Livewire (Layer 6) nie potrzebuje już tego zapisu do poprawności na PRAWDZIWYM żądaniu — Layer 7 sprawia, że OUTER `/livewire/update` sam rozwiązuje tenanta z realnego Hosta, zanim Livewire cokolwiek zdyspatchuje. Szczegóły: `app/docs/security/vulnerabilities/VULN-003-root-domain-tenant-bypass.md`, sekcja Layer 8.
