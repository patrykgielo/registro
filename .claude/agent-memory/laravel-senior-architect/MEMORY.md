...
## Wielooddziałowość / Lokalizacje — Faza 6 krok 6.2 (2026-09-18, branch feature/lokalizacje-faza6-zmiana-oddzialu)
- [project_lokalizacje_faza6_krok62_zmiana_oddzialu.md](project_lokalizacje_faza6_krok62_zmiana_oddzialu.md) — `CartService::setLocation()`/`previewLocationChange()`, non-empty-cart location switch confirmation. **2026-09-19 follow-up: fixed the gap this file itself flagged** — `CartController::add()` never forwarded a location, so every real add-to-cart/checkout validated against the GLOBAL pool, not the customer's selected branch; `addItem()` now derives its location from `$cart->location_id`, new `syncItemLocationsToCart()` heals legacy rows. Wrong-then-fixed concurrency test design worth reading before writing a new one: a bare CartItem is never a reservation in this system, racing two non-checked-out carts against each other proves nothing — race a real checkout against a confirmation instead. **Same-day 3rd incident:** `addItem()`/`updateQuantity()` still didn't LOCK the cart row (only setLocation()/convertToOrder() did) — fixed with the same `Cart::where(...)->lockForUpdate()->firstOrFail()` cart-first-then-services order; falsifying it reproduced a real InnoDB deadlock, not a quiet mismatch — see file for why that's still a valid falsification.

## Wielooddziałowość / Lokalizacje — onboarding/magazyn bugs (2026-09-19, branch feature/lokalizacje-onboarding-stan)
- [project_lokalizacje_onboarding_stan_fixes.md](project_lokalizacje_onboarding_stan_fixes.md) — 3 ClickUp bugs, zero migracji (zmierzone: 0/8 orgs bez lokalizacji, 0/26 usług bez stanu). Provisioning teraz zakłada "Siedziba główna"; seeder + command + `CreateService::afterCreate()` materializują `service_location_stocks`; `forLocation()` dziedziczy `quantity_total` gdy nowy oddział jest GENUINE pierwszym. **Blanket `Service::created()` observer odrzucony** — koliduje z `unique(service_id,location_id)` w ~8 plikach testowych używających wzorca "utwórz usługę, potem ręcznie wstaw asymetryczny split" (potwierdzone empirycznie, nie domysłem). Pierwszy egzemplarz przy istniejącej ręcznej ilości materializuje różnicę jako bezimienne placeholdery zamiast zerować kotwicę — złapano po drodze realny bug: `$unit->status` bywa `null` w pamięci mimo `DEFAULT 'available'` w DB (Eloquent nie odczytuje defaultów z powrotem po `create()`)
