...
## Storefront walkthrough (Layer 2) — real cross-tenant leak found (added 2026-08-29)
- [project_storefront_walkthrough_substitute_bindings_leak_2026-08-29.md](project_storefront_walkthrough_substitute_bindings_leak_2026-08-29.md) — `tests/Feature/StorefrontWalkthroughTest.php`, real-Host-header + real `ResolveTenant` middleware (not the usual bind-override test double) found that `SubstituteBindings` (implicit `{model:slug}` binding) runs BEFORE `ResolveTenant` for every route on a `BelongsToOrganization` model, because it's baked into Laravel's default `web` middleware group — a real, previously-undiscovered cross-tenant leak, orthogonal to VULN-003's documented layers. NOT fixed (out of task scope, reported to team-lead). Also documents the "compact assertion helper" pattern (avoid `assertSee()`'s full-page-dump failure messages inside an accumulate-violations test) and a reusable falsifiability-proof technique.

## Two-connection oversell race harness — tests/Concurrency (added 2026-08-27)
- [project_concurrency_harness_cart_checkout_race_2026-08-27.md](project_concurrency_harness_cart_checkout_race_2026-08-27.md) — Faza 0 krok 0.3 (multi-location plan): first two-OS-process, real-InnoDB test in the repo, `CartService::convertToOrder()`. Run via `bash scripts/test-concurrency.sh` (throwaway `mysql:8.0`, never dev DB). Measured, not guessed: `forUpdate: true` closes the race for the last-unit/overlapping-dates scenario; `Service::lockForUpdate()` alone does not — narrower than the "both required" framing in `kontrakt-dostepnosci.md` Zasada 4 / the `RentalAvailabilityService` docblock. Reusable technique: `proc_open` for real concurrency (one PHP process can't hold a lock open while a second blocks on it) + `DB::listen()` + file-signal for deterministic (not timing-raced) sequencing. Read before touching `CartService`'s lock discipline or writing another concurrency test.
...
- [project_stale_seeder_comments_2026-08-16.md](project_stale_seeder_comments_2026-08-16.md) — follow-up cleanup: 11 comments (8 in `tests/**`, 3 in `.claude/rules/**`/`app/docs/**`) still quoted the old per-test `TestCase::setUp()` seeding after the PR above shipped. Fixed all; verified per-file that none of the 8 tests were weakened by the mechanism switch; found one unrelated pre-existing mislabeled assertion in `OrderPaidPickupHtmlSeparatorMigrationTest` while checking.
